...
public class TranslateController {
	...
	/**
	 * Este metodo esta en desarrollo Temporalmente es sustituido
	 * por @dictionaryTest()
	 */
	...
	public static Translator readDataTypeGuided() {
		...
		System.out.println(
				"Ahora definimos los cred1itos por metal, el valor debe ser numerico, recuerda que si es decimal va separado por coma");
	...
	public static List<String> readTextToTranslate() {
		...
		System.out.println("Cuánta madera podría arrojar una marmota si una marmota pudiera arrojar madera");
		...
		do {
			...
			if (str.length() > 10) {
				if (str.substring(0, 10).equalsIgnoreCase("Cuánto es ")) {
					result.add(str);
				} else if (str.length() > 23 && str.substring(0, 23).equalsIgnoreCase("Cuántos créditos tiene ")) {
					result.add(str);
				} else {
					result.add(str);
				}
	...
	public static List<String> textToTranslateTest() {
		...
		result.add("Cuánta madera podría arrojar una marmota si una marmota pudiera arrojar madera");
